...
current_backend_data_json() {
	local manifest
	manifest="$(jq -r '.backendData.quadlet.manifestPath // empty' "$podman_compose_metadata" 2>/dev/null || true)"
	if [ -n "$manifest" ] && [ -r "$manifest" ]; then
		jq -cn --slurpfile manifest "$manifest" '{kind: "quadlet", quadlet: $manifest[0].quadlet}'
		return
	fi
...
backend_transition_quadlet_fragment_owns_source() {
	local fragment_path="$1" expected_source_path="$2" line
	[ -r "$fragment_path" ] || return 1
	while IFS= read -r line; do
		[ "$line" = "SourcePath=$expected_source_path" ] && return 0
	done <"$fragment_path"
	return 1
}

backend_transition_quadlet_unit_clean() {
	local active_state container_unit encoded fragment_path generated_fragment load_state source_path source_path_actual state prior_data
	prior_data="$1"
	mapfile -t prior_quadlet_units < <(jq -r '
		.quadlet as $quadlet
		| if (($quadlet.units // []) | length) > 0 then $quadlet.units[] | @base64
		  elif ($quadlet.containerUnit // "") != "" then {
			unit: $quadlet.containerUnit,
			sourcePath: $quadlet.sourcePath
		  } | @base64
		  else empty
		  end
	' <<<"$prior_data")
	[ "${#prior_quadlet_units[@]}" -gt 0 ] || {
		printf 'prior Quadlet unit identity is missing for backend transition of %s\n' \
			"$podman_compose_service_name" >&2
		return 1
	}
	for encoded in "${prior_quadlet_units[@]}"; do
		container_unit="$(base64 -d <<<"$encoded" | jq -r '.unit // empty')"
		source_path="$(base64 -d <<<"$encoded" | jq -r '.sourcePath // empty')"
		[ -n "$container_unit" ] && [ -n "$source_path" ] || return 1
		...
		not-found:inactive) continue ;;
		loaded:inactive)
			if [ "$source_path_actual" = "$source_path" ]; then
				generated_fragment="$fragment_path"
			elif [ "$fragment_path" = "$source_path" ]; then
				generated_fragment="$source_path_actual"
			else
				return 1
			fi
			case "$generated_fragment" in
			"$runtime_dir/systemd/generator/$container_unit" | \
				"$runtime_dir/systemd/generator.early/$container_unit" | \
				"$runtime_dir/systemd/generator.late/$container_unit")
				backend_transition_quadlet_fragment_owns_source \
					"$generated_fragment" "$source_path" || return 1
				continue
				;;
			esac
			;;
