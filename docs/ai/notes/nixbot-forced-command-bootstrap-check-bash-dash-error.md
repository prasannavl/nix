...
- Observed during local deploy run on 2026-02-26:
  ...
  - `bash: --: invalid option`
- This happened in `scripts/nixbot-deploy.sh` while running `check_bootstrap_via_forced_command`.

## Root Cause
- The forced-command bootstrap check sent raw option-style arguments over SSH (`--sha`, `--hosts`, ...).
- In non-forced-command shell paths, this can be interpreted by `bash -c` as an invocation option stream, producing:
  - `bash: --: invalid option`
...
- Added SSH original-command normalization in `main()`:
  ...
  - This keeps forced-command parsing robust for both legacy and explicit-script invocations.

## Expected Behavior After Change
- Forced-command bootstrap checks run via an explicit script command and avoid bash option parsing failures.
