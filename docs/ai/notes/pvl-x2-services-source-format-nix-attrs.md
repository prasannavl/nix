...
- Compose content is now expressed as Nix attrsets so the module renderer emits
  YAML.
...
- Converted `services.immich.files."hwaccel.ml.yml"` and
  `"hwaccel.transcoding.yml"` from YAML strings to Nix attrsets.
