...
- Provide an operational path to rotate bastion in one pass while retaining old
  deploy private key only for old nodes in phase 2.
...
- Updated modules:
  ...
  - `lib/nixbot/bastion.nix` now installs all forced-command keys from
    `bastionSshKeys`.
...
- Documented in `docs/deployment.md`:
  ...
  - bastion-first single-pass cutover using per-host legacy deploy key overrides
    in `hosts/nixbot.nix`
