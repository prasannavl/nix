...
- Cover overlap rotation and bastion-first cutover with legacy-node bootstrap
  handling.
...
- Added `docs/ai/playbooks/nixbot-key-rotation-execution.md`:
  ...
  - Mode B: bastion-first single-pass cutover with per-host `key` +
    `bootstrapKey` legacy overrides
