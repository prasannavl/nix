...
## Context
- Task: update GitHub CI workflow to use Tailscale trust credentials (OIDC federation) instead of deprecated auth key input.
...
- Ensure the Tailscale trust credential (OIDC) is configured for GitHub issuer and matching subject for this repo/environment.
- Ensure `tag:ci` is allowed for this trust credential in Tailscale policy/tag owners.
