...
changes could alter the stamp even when the declarative tag value itself had
not changed.
...
- `bootTag` and `recreateTag` now affect reconciliation only when their
  declared values change
