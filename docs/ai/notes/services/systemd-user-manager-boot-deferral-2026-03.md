...
- `dry-activate` runs preview logging for reconcile and skips prune and identity
  refresh entirely
...
starts only after a successful apply. Boot-gated consumers can attach their user
services to that target instead of `default.target`.
...
- boot-gated managed user services no longer race ahead of the reconciler in the
  user manager
