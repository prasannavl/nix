...
- Keep boot-time auto-reconcile opt-in via `services.incusMachines.autoReconcile`
  instead of coupling it to activation or boot success.
...
- Those command templates live in host metadata/defaults rather than being
  hard-coded into `nixbot` terminology, so the orchestration model stays
  generic even though the current implementation is Incus-backed.
