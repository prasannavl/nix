...
- Tailscale auth wiring lives in the shared `lib/incus-machine.nix` path, not as
  ad hoc host-local setup.
...
- Secret wiring should tolerate the encrypted file not existing yet by gating it
  with `builtins.pathExists`.
- The reusable factory defaults the secret name from `hostName` but still allows
  an explicit override for future guests.
