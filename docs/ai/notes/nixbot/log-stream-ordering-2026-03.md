...
`nixos-rebuild-ng` prints some final status messages across two streams: the
text prefix goes to stderr while the result path goes to stdout. When `nixbot`
...
Add a small `run_with_combined_output` helper in `scripts/nixbot-deploy.sh` that
performs `exec 2>&1` inside the child subshell before invoking the target
