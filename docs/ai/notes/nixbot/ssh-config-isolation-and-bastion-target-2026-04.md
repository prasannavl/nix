...
- `hosts/nixbot.nix` should use the real bastion hostname `z.gap3.ai` for
  `gap3-gondor` instead of the local alias `gap3-gondor`.

## Failure mode addressed

- A local `~/.ssh/config` entry mapped `gap3-gondor` to `z.gap3.ai`.
...
- That produced host-key verification failures against `z.gap3.ai` during
