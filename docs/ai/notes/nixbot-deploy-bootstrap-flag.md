...
## Context
- Request: add a CLI option to `scripts/nixbot-deploy.sh` to always force bootstrap path selection.
...
## Result
- Deploy/snapshot/rollback SSH target selection can be forced to bootstrap path regardless of primary target reachability.

## Follow-up Clarification
- Bootstrap remains automatic on normal deploy path (primary probe -> forced-command check -> bootstrap injection fallback).
