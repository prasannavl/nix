...
- `core.nix` — foundational system config: boot, networking, security, locale,
  ...
  tools.
- `incus-lxc.nix` — minimal profile for Incus/LXC guests with no physical
  hardware assumptions.
- `incus-vm.nix` — profile for Incus VM guests.
- `vm.nix` — generic non-Incus VM profile.
...
Physical machines typically use `all.nix`. Incus guests select a machine profile
from `hosts/default.nix`, so guest-specific bootstrap modules are added
centrally instead of repeated in each host directory.
...
| `lib/profiles/incus-*.nix` | Incus guest machine profiles                |
...
Incus guests are registered in `hosts/default.nix` with an Incus machine profile
and are declared on their parent host via `lib/incus/default.nix`. They are full
NixOS hosts in `hosts/` with their own directory, registered in
`hosts/default.nix` and `hosts/nixbot.nix` like any other host. See
`docs/incus-vms.md` for the full lifecycle model.
...
Incus guests join automatically too, but through a different mechanism. The
Incus machine profiles check for a Tailscale auth key at
...
The optional Tailscale helper defaults to the hostname-derived secret file name,
and the profile skips Tailscale setup when that encrypted file is absent.
...
- Incus/container guests: `lxc.nix`
...
- `lib/profiles/incus-*.nix` (Tailscale auto-wiring for Incus guests)
