(() => {
  ...
  function labelBase(applicationId, applications) {
    return `${
      slugPart(applicationBindName(applicationId, applications)) || "app"
    }-device`;
  }
  ...
  function purposeVariant(purpose, variant) {
    ...
    const key = Object.keys(purpose).find((item) =>
      item.toLowerCase() === variant
    );
  ...
  function renderUnlockPrompt() {
    return node("div", {
      class:
        "alert alert-warning d-flex flex-column flex-sm-row align-items-sm-center justify-content-between gap-3",
      role: "alert",
    }, [
      node("div", {
        text:
          "This session is read-only. Unlock edits before changing app passwords.",
      }),
      node("a", {
  ...
  function getCurrentAccountId(whoami) {
    return whoami?.youare?.attrs?.name?.[0] ||
      whoami?.youare?.attrs?.spn?.[0]?.split("@")?.[0];
  }
  ...
  function renderExistingPasswords(
    ...
    const orderedPasswords = sortedPasswords(
      passwords,
      applications,
      sortState,
    );
    ...
    function renderPasswordActions(password) {
      ...
      regenerateButton.addEventListener("click", async () => {
        if (
          !window.confirm(
            `Regenerate application password "${password.label}"? The old password will stop working immediately.`,
          )
        ) {
          return;
        }
        ...
        try {
          ...
          window.alert(
            `Failed to regenerate application password: ${err.message}`,
          );
        }
      });
      deleteButton.addEventListener("click", async () => {
        if (
          !window.confirm(`Delete application password "${password.label}"?`)
        ) {
          return;
        }
        deleteButton.disabled = true;
        ...
        try {
          await requestJson(
            `/scim/v1/Person/${encodeURIComponent(accountId)}/Application/${
              encodeURIComponent(password.uuid)
            }`,
            {
              method: "DELETE",
            },
      ...
      return node("div", {
        class:
          "d-flex flex-wrap gap-2 justify-content-start justify-content-md-end",
      }, [
        regenerateButton,
        deleteButton,
      ]);
    ...
    orderedPasswords.forEach((password) => {
      tbody.append(node("tr", {}, [
        ...
        node("td", {
          text: applicationName(password.applicationUuid, applications),
        }),
        node("td", {
    ...
    const mobileCards = node(
      "div",
      { class: "d-xl-none" },
      orderedPasswords.map((password) => {
        return node("div", { class: "border-top py-3" }, [
          node("div", { class: "row g-2" }, [
            ...
            node("div", { class: "col-12 col-sm-6" }, [
              renderPasswordField(
                "Application",
                applicationName(password.applicationUuid, applications),
              ),
            ]),
  ...
  async function renderAppPasswords() {
    ...
    document.querySelectorAll(".side-menu-item").forEach((item) => {
      item.classList.toggle(
        "active",
        item.dataset.abirdAppPasswords === "true",
      );
    });
    ...
    try {
      ...
      setMessage(
        root,
        `Unable to load application password settings: ${err.message}`,
        "danger",
      );
      return;
    }
    ...
    if (!applicationList.length) {
      setMessage(
        root,
        "No applications are available for application passwords.",
        "warning",
      );
      return;
    }
    ...
    function updateDefaultLabel(force = false) {
      const nextLabel = nextDefaultLabel(
        appSelect.value,
        applicationList,
        visiblePasswords,
      );
      ...
      if (
        force || !labelTouched || labelInput.value.trim() === "" ||
        labelInput.value === previousDefaultLabel
      ) {
    ...
    root.replaceChildren(
      ...(!writeState.writable && writeState.canUnlock
        ? [renderUnlockPrompt()]
        : []),
      node("div", { class: "mb-4" }, [
        node("p", {
          ...
          text:
            "App passwords are compatibility credentials for apps that do not yet support passkeys.",
        }),
        ...
        node("form", { class: "row gy-3" }, [
          ...
          node("div", { class: "col-12 d-flex justify-content-end" }, [
            generateButton,
          ]),
        ]),
      ]),
    ...
    labelInput.addEventListener("input", () => {
      labelTouched = labelInput.value.trim() !== "" &&
        labelInput.value !== currentDefaultLabel;
    });
    ...
    function showGeneratedPassword(password, applicationUuid, message) {
      status.replaceChildren(
        node("div", { class: "alert alert-success", role: "alert" }, [
          node("p", {
            class: "mb-2",
            text: message,
          }),
          ...
          node("div", {
            class: "form-text",
            text: `Bind DN: name=${accountId},app=${
              applicationBindName(applicationUuid, applicationList)
            }`,
          }),
        ]),
      );
    ...
    async function regenerateApplicationPassword(password) {
      ...
      const replacement = await createApplicationPassword(
    ...
    async function refreshExistingPasswords() {
      try {
        const person = await requestJson(
          `/scim/v1/Person/${encodeURIComponent(accountId)}`,
        );
        ...
        setMessage(
          existingPasswords,
          "Existing application passwords are not visible in this session. Generated passwords will still be shown once at creation time.",
          "secondary",
        );
      }
    }
    ...
    generateButton.addEventListener("click", async () => {
      ...
      const label = labelInput.value.trim() || currentDefaultLabel ||
        labelInput.placeholder;

      try {
        const password = await createApplicationPassword(
          selectedApplication,
          label,
        );
        showGeneratedPassword(
        ...
        status.replaceChildren(
          node("div", { class: "alert alert-danger", role: "alert" }, [
  ...
  function ensureMenuItem() {
    const sideMenu = document.querySelector(".side-menu");
    if (
      sideMenu && !sideMenu.querySelector("[data-abird-app-passwords='true']")
    ) {
      const credentials = [...sideMenu.querySelectorAll("a")].find((item) =>
        item.href.endsWith("/ui/update_credentials")
      );
      const listItem = node("li", {}, [
        node("a", {
          ...
          node("div", {
            class:
              "icon-container align-items-center justify-content-center d-flex me-2",
          }, [
            node("img", {
              alt: "",
              class: "text-body-secondary",
    ...
    const signOutLink = [...document.querySelectorAll("nav a")].find((item) => {
      return normalizeText(item.textContent) === "sign out" ||
        item.href.endsWith("/ui/logout");
    });
    const referenceLink = profileLink ?? signOutLink;
    if (
      !nav || !referenceLink ||
      document.querySelector("nav [data-abird-app-passwords='true']")
    ) {
      return;
    }
  ...
  function reconcileActiveState() {
    document.querySelectorAll("[data-abird-app-passwords='true']").forEach(
      (item) => {
    ...
    document.querySelectorAll(
      ".side-menu-item:not([data-abird-app-passwords='true'])",
    ).forEach((item) => {
