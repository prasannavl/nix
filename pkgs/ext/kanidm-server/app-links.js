...
const metadataByDisplayName = new Map(
  appLinks.map((app) => [normalizeText(app.displayName), app]),
);
const groupOrder = [
  { id: "core", label: "Core" },
...
const groupMetadata = new Map(
  groupOrder.map((group, index) => [group.id, { ...group, index }]),
);
...
function appFromHref(href) {
  ...
  for (const app of appLinks) {
    if (
      href.includes(`/oauth2/openid/${app.name}`) ||
      href.includes(`/ui/oauth2/${app.name}`)
    ) {
      return app;
    }
  }
...
function appFromText(container) {
  ...
  return metadataByDisplayName.get(normalizeText(container.textContent)) ??
    null;
}
